...
namespace ESPExplorerAE
{
    ...
    void ContextMenu::Draw(const FormEntry& entry, const ContextMenuCallbacks& callbacks)
    {
        ...
        if (callbacks.showSpawnAtPlayer && (CanGiveItem(entry.category) || canSpawnEntry)) {
            ...
            ImVec4 frameBgHovered = ImGui::GetStyleColorVec4(ImGuiCol_FrameBgHovered);
            ImVec4 frameBgActive = ImGui::GetStyleColorVec4(ImGuiCol_FrameBgActive);
            frameBg.w = (std::max)(frameBg.w, 0.30f);
            frameBgHovered.w = (std::max)(frameBgHovered.w, 0.36f);
            frameBgActive.w = (std::max)(frameBgActive.w, 0.42f);
